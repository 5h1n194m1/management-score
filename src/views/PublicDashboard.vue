...
const publicScores = ref(null);
const isLoading = ref(true);

const fetchPublicData = async () => {
    isLoading.value = true;
    // --- Lakukan Fetch Data DARI API yang TIDAK membutuhkan token Auth ---
    // Contoh: Hanya peringkat publik dan data yang diizinkan untuk dilihat umum
    await new Promise(resolve => setTimeout(resolve, 500)); // Simulasi loading
    publicScores.value = {
        eventName: 'Kompetisi IT Nasional 2025',
        leaderboard: [
            { rank: 1, team: 'Tim Alpha', score: 95 },
            { rank: 2, team: 'Tim Beta', score: 88 },
            { rank: 3, team: 'Tim Gamma', score: 79 },
        ]
    };
  ...
  <div class="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-10">
    <div class="w-full max-w-4xl bg-white p-10 rounded-xl shadow-2xl text-center space-y-8">
        ...
        <h1 class="text-4xl font-extrabold text-indigo-700">
            {{ publicScores ? publicScores.eventName : 'Sistem Manajemen Skor' }}
        </h1>
        
        <p class="text-xl text-gray-600">
            Selamat datang di papan peringkat publik.
        </p>
        ...
        <div v-else class="space-y-6">
             <h2 class="text-2xl font-bold text-gray-800 border-b pb-2">Papan Peringkat Top 3</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        <tr v-for="item in publicScores.leaderboard" :key="item.rank" 
